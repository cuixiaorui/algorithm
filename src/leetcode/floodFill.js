...
var floodFill = function (image, sr, sc, newColor) {
    ...
    if(image[sr] === null)return;
    if(image[sr][sc] === null)return;
    ...
    while(queue.length !== 0){
        ...
        const points = getPoints(curPoint);
        points.push(curPoint)

        for(let i=0,len = points.length; i<len; i++){
            const nPoint = points[i]
            const {x,y} = nPoint;
            console.log(image[x][y]);
            if(image[x][y] === target && visited[x][y] === 0){
                image[x][y] = newColor;
                queue.push(nPoint);
                visited[x][y] = 1;
            }
